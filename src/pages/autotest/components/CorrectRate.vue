    ...
    <el-card class="box-card">
      ...
      <div class="correct-rate-table">
        ...
        >
          ...
          <el-table-column prop="first_word_correct_rate" label="首词正确率" align="center"></el-table-column>
          <el-table-column prop="first_page_correct_rate" label="首屏正确率" align="center"></el-table-column>
      ...
      >
        <el-form ref="form" :model="form" :rules="rules" label-width="150px" label-position="left">
          ...
          <el-form-item label="首词正确率">
            <el-input v-model="form.first_word_correct_rate" placeholder="请输入内容"></el-input>
          </el-form-item>
          <el-form-item label="首屏正确率">
            <el-input v-model="form.first_page_correct_rate" placeholder="请输入内容"></el-input>
          </el-form-item>
  ...
  methods: {
    ...
    editItemSure(params) {
      this.$refs["form"].validate(valid => {
        if (valid) {
          monkeyApi
            .editFirstList(params, params.id)
            .then(res => {
    ...
    getChars(data) {
      ...
      console.log(data);
      let xAxis = [];
      let first_word_correct_rate = [];
      let first_page_correct_rate = [];
      data.forEach(item => {
        xAxis.push(item.apk_version);
        first_word_correct_rate.push(item.first_word_correct_rate);
        first_page_correct_rate.push(item.first_page_correct_rate);
      });
        ...
        series: [
          {
            name: "首词正确率",
            type: "line",
            data: first_word_correct_rate
          },
          {
            name: "首屏正确率",
            type: "line",
            data: first_page_correct_rate
          }
        ]
      };
